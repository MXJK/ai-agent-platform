...
- 对话页将空会话引导与活动会话工作台分开：开始对话后切换为紧凑会话标题和独立消息
  滚动区，输入框固定显示当前 Workspace、模型、会话累计实际 Token 消耗、模型单次请求
  上下文上限，以及“累计消耗 / 上下文上限”的百分比。该比例可跨多轮超过 100%，不代表
  当前最终 Prompt 已超出上下文窗口；会话详情仍单独展示保留消息与摘要的上下文估算。
  用户消息可复制或重新编辑，
  助手消息可复制，失败响应提供重试、更换模型和 Trace 审计入口。顶部栏、活动会话标题和
  输入区采用紧凑密度，让消息区获得更多可视高度；
...
预算、是否含摘要、丢弃与截断条数、同步压缩次数。会话 Token API 按当前会话模型偏好

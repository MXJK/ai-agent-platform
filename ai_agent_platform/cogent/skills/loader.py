...
import yaml
from ai_agent_platform.cogent.skills.parser import SkillDef, SkillParseError, resolve_mode_and_context, parse_skill_file
log = logging.getLogger(__name__)
PROJECT_SKILLS_DIR = '.cogent/skills'
USER_SKILLS_DIR = '~/.cogent/skills'
...
class SkillLoader:
    ...
    def _parse_skill_yaml(yaml_path: Path, skill_dir: Path) -> SkillDef | None:
        ...
        if mode not in ('inline', 'fork'):
            mode = 'inline'
        return SkillDef(name=name, description=description, prompt_body=prompt_body, mode=mode, model=meta.get('model'), context=context, source_path=prompt_md if prompt_md.is_file() else yaml_path, is_directory=True)
    ...
    def get(self, name: str) -> SkillDef | None:
        ...
        if skill.source_path is not None:
            try:
                ...
                log.warning("Hot-reload failed for skill '%s', using cached version: %s", name, e)

...
class ToolPoolBuilder:
    ...
    def restore(self, snapshot: object) -> EffectiveToolPool:
        ...
        try:
            canonical_catalog_summary = _canonical_json(
                json.loads(catalog_summary)
            )
        except (TypeError, ValueError) as exc:
...
def _entry_from_spec(
    ...
    *,
    ...
    resolved_source: ToolSource = source or (
        "mcp" if spec.provider.startswith("mcp:") else "local"
    )
    namespace = "mcp" if resolved_source == "mcp" else (
        spec.name.split(".", 1)[0] if "." in spec.name else "local"
    )

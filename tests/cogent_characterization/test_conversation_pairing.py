...
from ai_agent_platform.cogent.conversation import Message, ToolResultBlock, ToolUseBlock
from ai_agent_platform.cogent.conversation_pairing import INTERRUPTED_TOOL_RESULT, ensure_tool_pairing

def assistant_with_tool(tool_id: str) -> Message:
    return Message(role='assistant', content='let me check', tool_uses=[ToolUseBlock(tool_use_id=tool_id, tool_name='ReadFile', arguments={})])

def result_for(tool_id: str, content: str) -> Message:
    return Message(role='user', content='', tool_results=[ToolResultBlock(tool_use_id=tool_id, content=content, is_error=False)])

def test_paired_history_is_untouched() -> None:
    got = ensure_tool_pairing([Message(role='user', content='hi'), assistant_with_tool('t1'), result_for('t1', 'content')])
    assert len(got) == 3
...
def test_orphan_result_is_dropped() -> None:
    got = ensure_tool_pairing([Message(role='user', content='hi'), result_for('ghost', 'leftover'), Message(role='assistant', content='ok')])
    assert len(got) == 2
    for m in got:

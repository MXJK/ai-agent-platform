...
from ai_agent_platform.repositories.workspace_access import (
...
class PostgresRepositoryTests(unittest.TestCase):
    ...
    def test_workspace_membership_promotes_roles_without_downgrade(self) -> None:
        now = datetime(2026, 7, 30, tzinfo=timezone.utc)
        connection = FakeConnection(
            [("project", "owner", "admin", now, now)]
        )
        with patch(
            "ai_agent_platform.repositories.workspace_access._require_psycopg",
            return_value=object(),
        ):
